...
stripe_lib.redirectToCheckout({
  ...
  sessionId: data
	...
	alert("Невозможно переадресовать на Stripe");
  // If `redirectToCheckout` fails due to a browser or network
  // error, display the localized error message to your customer
  // using `result.error.message`.
});
						alert('Пожалуйста, подождите, мы соединяем вас с Stripe');
				}
			});
...
return;
		    ...
		    $.ajax({
    	       ...
    	       success: function(data)
        	   {
					if (data == 'Error') {
						...
						alert('Пожалуйста, подождите, мы подключаем вас к PayPal');
					} else {
						self.location = '/invoice/'+data+'/success';
					}
	           }
	...
	$('.apply_gc').on('click', function() {
		...
		$('.coupon_screen').html('<br/><div class="group"><input type="text" size="30" required /><span class="highlight"></span><span class="bar"></span><label>Введите код для подарочной карты здесь</label></div><div class="coupon-error"></div><button>Применить</button><br/><br/>');
	...
	$('.apply_coupon').on('click', function() {
		bc = false;
		var id = alert('<div class="coupon_screen"></div>', 1, '', 'coupon_alert');
		$('.coupon_screen').html('<div class="group"><input type="text" size="30" required /><span class="highlight"></span><span class="bar"></span><label>Введите код купона здесь</label></div><div class="coupon-error"></div><button>Применить</button><br/><br/>');
		$('.coupon_screen input').on('keyup', function() {
			$('.coupon-error').hide();
		});
...
function instant_search() {
	$('.search input').unbind('focus').on('focus', function() {
		var val = $(this).val();
		if (val.length < 2) {
			$('.instant-search').html("<div class='enter-3-chars'>Введите не менее 2 символов</div>");
			return;
		}
	...
	$('.search').unbind('mouseover').mouseover(function() {
		...
		if (val.length < 2) {
			$('.instant-search').html("<div class='enter-3-chars'>Введите 2 символа</div>");
			return;
		}
...
function close_ticket(id, what) {
	if (what == '1') {
		$('#ticket-status-'+id).html("Закрытые");
		var sid = 'C';
		alert('Ticket closed');
	} else {
		$('#ticket-status-'+id).html("Отменен");
