...
function product_options(o) {
	...
	if ($(qadd+'#hasVariants').length > 0) {
		...
		if (variantid > 0) {
			...
			html += '<td nowrap><a href="javascript: void(0);" class="clearAll">Очистить все</a></td>';
			...
			if (cnt == cnt2) {
				...
				bc = false;
				alert("Такое сочетание опций недоступно");
			} else {
...
function map_variants(o, groupid) {
	...
	$(qadd+'.unavailable').each(function() {
		if (!$(this).parent().hasClass('novar')) {
			...
			$(qadd+'.options_container img.checked').each(function() {
				if (id != $(this).parent().parent().attr('id')) {
					html += '<tr><td>'+$(this).parent().attr('data-title')+'</td><td class="clear"><a href="javascript: void(0);" class="clear-option" id="co-'+$(this).parent().parent().attr('id').replace('pog-', '')+'">Очистить</a></td></tr>';
					found = true;
				}
			});
			...
			$(qadd+'.product_options').each(function() {
				if (id != $(this).attr('id').replace('po-', '')) {
					html += '<tr><td>'+$(this).find('option:selected').text()+'</td><td class="clear"><a href="javascript: void(0);" class="clear-option" id="co-'+$(this).attr('id').replace('po-', '')+'">Очистить</a></td></tr>';
					found = true;
				}
			});
		}
		...
		html += '</table>';
		if (found)
			$(this).append("<div class='help-option'>Опция недоступна в сочетании с"+html+'<img src="'+current_location+'/images/spacer.gif" alt="" /></div>');
		else
			$(this).append("<div class='help-option'>Опция недоступна<img src='"+current_location+"/images/spacer.gif' alt='' /></div>");
	});
...
function add_to_cart(productid, is_quick) {
	...
	$.ajax({
		...
		unload();
		if (r == "1") {
			alert("Пожалуйста, выберите варианты");
			return false;
		}

		if (mobile_design) {
			alert("Товар был добавлен в вашу корзину.<br /><br /><a href='/cart'><button>Корзина</button></a> &nbsp; <a href='/checkout'><button>Заказать</button></a>", 1);
		}
...
function add_review() {
	...
	$.ajax({
		...
		unload();
		if (r == "1") {
			alert("Пожалуйста, введите ваше имя и сообщение");
			return false;
		} else if (r == '2') {
			recaptchaOnload();
			alert("Captcha неверна");
			return false;
		} else if (r == '3') {
			recaptchaOnload();
...
function add_wishlist(id) {
	$.ajax({
		...
		alert("Товар был добавлен в список желаний");
	});
}

function buy_one_click(pid) {
	alert("Мы свяжемся с вами в ближайшее время<br /><input type='text' placeholder='"+lbl_buy1click+"' /> <a class='main-button'>Отправить</a>", 1, '', 'buy_one_click');
	$('.buy_one_click a.main-button').on('click', function() {
		...
		$.ajax({
			...
			if (r == 'N') {
				alert("Вы можете запрашивать быструю продажу не чаще одного раза в 10 секунд");
			} else {
				alert("Пожалуйста, дайте нам несколько минут для получения вашего запроса");
			}
		});
