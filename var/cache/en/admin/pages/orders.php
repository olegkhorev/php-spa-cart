...
<?php 
	...
	foreach ($orders as $k=>$v) {		if ($v['status'] == '2' || $v['status'] == '3' || $v['status'] == '6')
		...
		<tr>
			...
			<td width="100" class="no-word-break"><label>Status</label>
...
#exit(print_R($v));
		...
		echo '</td>
			<td nowrap><label>Customer</label><a href="/admin/user/'.$v['userid'].'">'.$v['firstname'].' '.$v['lastname'].' ('.$v['email'].')</a></td>
			<td width="130" nowrap><label>Date</label><a href="/admin/invoice/'.$v['orderid'].'">'.date($datetime_format, $v['date']).'</a></td>
			<td align="right"><label>Total</label><a href="/admin/invoice/'.$v['orderid'].'">$'.$v['total'].'</a></td>
		</tr>
		';
  	}
