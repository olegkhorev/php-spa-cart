...
function login_popup(what) {
  ...
  if (!what)
    ...
    if (what == 'W')
      ...
      alert('Sie müssen angemeldet sein, um auf diese Seite zuzugreifen');
...
function restore_password() {
	...
	alert("Ihre E-mail: <input placeholder=\"Geben Sie Ihre E-Mail-ersten\" type='text'><div></div>", 1, '', 'res_p');
	...
	$('.res_p input').on('keyup', function() {
		$.ajax({url: '/res_p?e='+$(this).val()}).done(function(r) {
			...
			$('.res_p button').on('click', function() {
				...
				alert("Link zur änderung des Passwortes an Ihre E-Mail");
			});
		});
	});
