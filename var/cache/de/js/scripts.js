...
return;
	...
	$('.apply_coupon').on('click', function() {
		bc = false;
		var id = alert('<div class="coupon_screen"></div>', 1, '', 'coupon_alert');
		$('.coupon_screen').html('<div class="group"><input type="text" size="30" required /><span class="highlight"></span><span class="bar"></span><label>Geben Sie Ihren Gutschein-code hier</label></div><div class="coupon-error"></div><button>Bewerben</button><br/><br/>');
		$('.coupon_screen input').on('keyup', function() {
			$('.coupon-error').hide();
		});
...
function close_ticket(id, what) {
	if (what == '1') {
		$('#ticket-status-'+id).html("Geschlossen");
		var sid = 'C';
		alert('Ticket closed');
	} else {
		$('#ticket-status-'+id).html("Abgesagt");
