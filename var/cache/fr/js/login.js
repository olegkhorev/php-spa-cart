...
function restore_password() {
	...
	alert("Votre E-mail: <input placeholder=\"Tapez votre premier e-mail\" type='text'><div></div>", 1, '', 'res_p');
	...
	$('.res_p input').on('keyup', function() {
		$.ajax({url: '/res_p?e='+$(this).val()}).done(function(r) {
			...
			$('.res_p button').on('click', function() {
				...
				alert("Lien pour changer le mot de passe envoyé à votre adresse email");
			});
		});
	});
