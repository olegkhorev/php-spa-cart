...
product_avail[<?php echo $oid;?>] = <?php echo $product['avail'];?>;
 ...
 <td class="details">
<?php if ($oid == 1) {?> <div class="see_all"><a class="ajax_link" href="<?php echo $current_location;?>/<?php echo $product['cleanurl'] ? $product['cleanurl'].'.html' : 'product/'.$product['productid'];; ?>">Voir plus de détails</a></div>
<?php } ?>
...
<tr>
 <td class="name">Liste de prix:</td>
...
<tr>
 ...
 <td class="product-option"><strong class="subtitle"><?php echo $v['fullname'] ? $v['fullname'] : $v['name'];; ?><span id="pot-<?php echo $v['groupid'];?>"></span> <a href="javascript: void(0);" class="clear_option" id="poa-<?php echo $v['groupid'];?>">Claire</a></strong><br />
...
<tr class="options-error">
 <td colspan="7" align="left">S'il vous plaît, sélectionnez toutes les options.</td>
</tr>
...
<?php if ($product['avail'] > 0) {?>
 ...
 <td id="out_of_stock"<?php if ($product['avail'] < 1) {?> class="shown"<?php } ?>>En rupture de stock</td>
 <td>
<input type="hidden" name="options_ex" value="" />
 <button class="add2cart add2cart-product" id="productid-<?php echo $product['productid'];?>">Ajouter au panier<span><svg><use xlink:href="/images/sprite.svg#cart"></use></svg></span></button>
 </td>
<?php } else  { ?>
 <td class="out-of-stock" colspan="3"> &nbsp; En rupture de stock</td>
<?php } ?>
<?php if ($oid) {?>
</tr>
<tr>
 <td colspan="4"><a class="main-button" onclick="javascript: return <?php if ($login) {?>add_wishlist(<?php echo $product['productid'];?>);<?php } else  { ?>login_popup('W');<?php } ?>" href="javascript: void(0);">Ajouter à la liste d'envies</a></td>
<?php } else  { ?>
...
Marque: 
<a class="main-button buy-with-one-click" onclick="javascript: buy_one_click(<?php echo $product['productid'];?>);" href="javascript: void(0);">Acheter en un seul clic</a>
<br /><br />
<a href="/ticket?productid=<?php echo $product['productid'];?>" class="ajax_link link-button">Posez une question sur ce produit</a>
<br /><br /><br />
<div class="price-breaks">
<span>Prix De Gros Sauts</span>
...
<ul class="product-tabs">
 ...
 <li class="tab-5" data-tab="5">Examens</li>
<?php if ($related_products) {?>
 <li class="tab-2" data-tab="2">Produits connexes</li>
<?php } ?>
<?php /* ?>
<?php if ($recommends) {?>
 <li class="tab-3" data-tab="3">Produits recommandés</li>
<?php } ?>
<?php */ ?>
 <li class="tab-4" data-tab="4">Envoyer à un ami</li>
</ul>
</div>
...
<h3 class="its4mobile">Examens</h3>
<?php if ($reviews) {?>
...
<?php } ?>
    <tr>
      <td colspan="2"></td>
      <td><br /><button type="button" onclick="javascript: add_review();">Ajouter un commentaire</td>
    </tr>
</table>
</form>
</div>
...
<h3 class="its4mobile">Produits connexes</h3>
...
<h3 class="its4mobile">Envoyer à un ami</h3>
...
<input type="hidden" name="mode" value="send_to_friend" />
  ...
  <table>
    ...
    <tr>
      <td colspan="2"></td>
      <td id="send_message_box"><label><input type="checkbox" id="is_msg" name="is_msg" value="1" onclick="javascript: $('.message-box').toggle();" value="Y"<?php if ($send_to_friend['is_msg']) {?> checked<?php } ?> />Ajouter un message personnel</label>
...
<?php } ?>
    ...
    <tr>
      <td colspan="2">&nbsp;</td>
      <td><br /><button type="button" onclick="javascript: send_to_friend();">Envoyer à un ami</td>
    </tr>
  </table>
</form>
</div>
