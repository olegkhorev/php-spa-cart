...
<h1 class="invoice-title"><span style="float: right;">{php echo date($datetime_format, $order['date']);}</span>{lng[Order]} #{$order['orderid']} ({$order_statuses[$order['status']]})</h1>
<h1 style="font-size: 12px; padding: 10px 0 0 0; margin: 0;">
{if $order['shipping_method']}
<span style="text-align:right; float: right;" class="delivery-method">{if $order['local_pickup']}{lng[Local pickup]}:{if $admin_display} {$order['warehouse']['wcode']},{/if} {$order['warehouse']['title']} ({$order['warehouse']['address']}){else}{lng[Delivery method]}: {$order['shipping_method']['shipping']}{if $order['shipping_method']['shipping_time']} ({$order['shipping_method']['shipping_time']}){/if}{/if}</span>
{/if}

{*
<span style="text-align:right; float: right;" class="delivery-method">{lng[Delivery method]}: {$order['shipping_method']['shipping']}{if $order['shipping_method']['shipping_time']} ({$order['shipping_method']['shipping_time']}){/if}</span>
*}
...
{if $config['Company']['orders_department']}Email: {$config['Company']['orders_department']}{/if}
 ...
 <td valign="top"><h2 style="margin: 0; padding: 10px 0; text-align: center;">{lng[Customer information]}</h2>
<table width="100%">
<tr>
...
{foreach $products as $v}
 {php $url = $v['cleanurl'] ? $v['cleanurl'] .'.html' : 'product/'.$v['productid'];}
	<tr>
	 <td class="image" valign="top" style="padding-right: 10px;"><a href="{$http_location}/{$url}">
{if $v['variant_photo']}
<?php
...
<tr>
 <td valign="top" width="100">{$o['name']}:</td>
 <td>{$o['option']['name']}</td>
</tr>
 {/foreach}
</table>
