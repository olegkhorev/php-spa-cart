...
<tr>
	...
	<td align="center"><label>{lng[Banner]}</label><a href="<?php echo $b['image_url'];?>" target="_blank"><img src="<?php echo $b['image_url'];?>" width="100"></a></td>
	<td><label>{lng[Link]}</label><input size="30" type="text" name="to_update[<?php echo $b['bannerid'];?>][url]" value="<?php echo escape($b['url']);?>" /></td>
	<td><label>{lng[Alt]}</label><input size="30" type="text" name="to_update[<?php echo $b['bannerid'];?>][alt]" value="<?php echo escape($b['alt']);?>" /></td>
	<td><label>{lng[Pos]}</label><input size="5" type="text" name="to_update[<?php echo $b['bannerid'];?>][pos]" value="<?php echo $b['pos'];?>"></td>
</tr>
<?php
	}
?>
...
?>
<tr>
 <td colspan="5" align="center" class="var-wh-100">{lng[No banners for this category]}</td>
</tr>
<?php
}
?>
...
<tr>
<td colspan="5" class="var-wh-100"><br /><h3>{lng[Add new]}</td>
</tr>

<tr>
	<td>&nbsp;</td>
	<td><label>{lng[Banner]}</label><input type="file" size="10" name="userfile" /></td>
	<td><label>{lng[Link]}</label><input size="30" type="text" name="new_url"></td>
	<td><label>{lng[Alt]}</label><input size="30" type="text" name="new_alt"></td>
	<td align="center"><label>{lng[Pos]}</label><input type="text" size="5" name="new_pos" /></td>
</tr>
...
<tr>
	<td height="10" nowrap="nowrap">{lng[Position]}</td>
	<td width="10" height="10">&nbsp;</td>
	<td height="10">
