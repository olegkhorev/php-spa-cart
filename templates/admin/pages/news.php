...
foreach ($newss as $b) {
	...
	<td><label>{lng[Title]}</label><a href="'.$current_location.'/admin/news/'.$b['newsid'].'">'.$b['title'].'</a></td>
	<td><label>{lng[Active]}</label><input type="checkbox" name="to_update['.$b['newsid'].'][active]" value="Y"'.($b['active'] == 'Y' ? ' checked="checked"' : '').' /></td>
</tr>';
}
?>
</table>
