...
foreach ($blogs as $b) {
	...
	<td><label>{lng[Title]}</label><a href="'.$current_location.'/admin/blog/'.$b['blogid'].'">'.$b['title'].'</a></td>
	<td><label>{lng[Active]}</label><input type="checkbox" name="to_update['.$b['blogid'].'][active]" value="Y"'.($b['active'] == 'Y' ? ' checked="checked"' : '').' /></td>
	<td align="center"><label>{lng[Comments]}</label><a href="'.$current_location.'/admin/blog/'.$b['blogid'].'">'.$b['comments']."</a></td>
</tr>";
}
?>
</table>
