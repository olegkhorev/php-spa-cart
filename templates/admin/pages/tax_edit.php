...
<?php
  ...
  <td align="center" nowrap="nowrap" class="no-word-break"><label>{lng[Tax rate value]}</label>
...
<select name="posted_data['.$t['rateid'].'][rate_type]">
  ...
  <td align="center"><label>{lng[Tax apply to]}</label><a href="/admin/taxes/'.$tax_details['taxid'].'?rateid='.$t['rateid'].'#rates">'.($t['formula'] == "" ? $tax_details['formula'] : $t['formula']).($t['shipping'] ? '+Shipping' : '').'</a></td>
</tr>
		';
	}
?>
