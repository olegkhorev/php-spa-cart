...
<li><a href="{$current_location}/admin/shipping?mode=add_realtime_methods">{lng[Manage realtime shipping methods]}</a></li>
<li class="active"><a href="javascript: void(0);">{lng[Shipping options|escape]}</a></li>
<li><a href="{$current_location}/admin/shipping_charges">{lng[Shipping charges]}</a></li>
</ul>
<br />

<h1>{lng[Shipping options]}</h1>
...
<tr>
  <td width="30%"><b>{lng[Carrier type]}:</b></td>
  <td width="70%">
...
<tr>
    <td colspan="2"><br /><h3>{lng[Package limits]}</h3></td>
</tr>
...
<tr>
  <td>
    <b>{lng[Maximum package weight]} (<?php echo $config['General']['weight_symbol']; ?>):</b>
  </td>
  <td nowrap="nowrap">
...
<tr>
  <td><b>{lng[Maximum package dimensions]} (<?php echo $config['General']['dimensions_symbol']; ?>):</b></td>
  <td nowrap="nowrap">
...
<tr>
  <td><label for="param01"><b>{lng[Split the shipment into multiple packages if its weight/dimensions exceed the limitations]}:</b></label></td>
...
<tr>
    <td><b>{lng[COD value]} (<?php echo $shipping_options['fdx']['currency_code'] ? $shipping_options['fdx']['currency_code'] : 'USD'; ?>):</b></td>
    <td>
...
<tr>
    <td><b>{lng[COD type]}:</b></td>
    <td>
        <select name="cod_type">
      <option value="ANY"<?php if ($shipping_options['fdx']['cod_type'] == "ANY") echo 'selected="selected"'; ?>>{lng[Any]}</option>
      <option value="GUARANTEED_FUNDS"<?php if ($shipping_options['fdx']['cod_type'] == "GUARANTEED_FUNDS") echo 'selected="selected"'; ?>>{lng[Guaranteed funds]}</option>
      <option value="CASH"<?php if ($shipping_options['fdx']['cod_type'] == "CASH") echo 'selected="selected"'; ?>>{lng[Cash]}</option>
        </select>
    </td>
</tr>

<tr>
    <td colspan="2"><br /><h3>{lng[Special services]}</h3></td>
</tr>

<tr>
  <td><b>{lng[Dangerous Goods/Accessibility]}:</b></td>
  <td>
  <select name="dg_accessibility">
    <option value=""<?php if ($shipping_options['fdx']['dg_accessibility'] == "") echo 'selected="selected"'; ?>>&nbsp;</option>
    <option value="ACCESSIBLE"<?php if ($shipping_options['fdx']['dg_accessibility'] == "ACCESSIBLE") echo 'selected="selected"'; ?>>{lng[Accessible dangerous goods]}</option>
    <option value="INACCESSIBLE"<?php if ($shipping_options['fdx']['dg_accessibility'] == "INACCESSIBLE") echo 'selected="selected"'; ?>>{lng[Inaccessible dangerous goods]}</option>
  </select>
  </td>
</tr>

<tr>
  <td><b>{lng[Signature option]}:</b></td>
  <td>
  <select name="signature">
    <option value=""<?php if ($shipping_options['fdx']['signature'] == "") echo 'selected="selected"'; ?>>&nbsp;</option>
    <option value="NO_SIGNATURE_REQUIRED"<?php if ($shipping_options['fdx']['signature'] == "NO_SIGNATURE_REQUIRED") echo 'selected="selected"'; ?>>{lng[No signature]}</option>
    <option value="INDIRECT"<?php if ($shipping_options['fdx']['signature'] == "INDIRECT") echo 'selected="selected"'; ?>>{lng[Indirect signature required]}</option>
    <option value="DIRECT"<?php if ($shipping_options['fdx']['signature'] == "DIRECT") echo 'selected="selected"'; ?>>{lng[Direct signature required]}</option>
    <option value="ADULT"<?php if ($shipping_options['fdx']['signature'] == "ADULT") echo 'selected="selected"'; ?>>{lng[Adult signature required]}</option>
  </select>
  </td>
</tr>
...
<tr>
  ...
  <tr>
    ...
    <td width="50%"><b><label for="dry_ice">{lng[Shipment contains dry ice]}</label></b></td>
    <td width="20">&nbsp;</td>
    ...
    <td width="50%"><b><label for="hold_at_location">{lng[Shipment is Hold at Location]}</label></b></td>
  </tr>
  ...
  <tr>
    ...
    <td><b><label for="inside_pickup">{lng[Shipment is Inside Pickup]}</label></b></td>
    <td>&nbsp;</td>
    ...
    <td><b><label for="inside_delivery">{lng[Shipment is Inside Delivery]}</label></b></td>
  </tr>
  ...
  <tr>
    ...
    <td><b><label for="saturday_pickup">{lng[Shipment is scheduled for Saturday pickup]}</label></b></td>
    <td>&nbsp;</td>
    ...
    <td><b><label for="saturday_delivery">{lng[Shipment is scheduled for Saturday delivery]}</label></b></td>
  </tr>
  ...
  <tr>
    ...
    <td><b><label for="residential_delivery">{lng[Shipment is Residential Delivery]}</label></b>
    </td>
    <td>&nbsp;</td>
    ...
    <td valign="top"><b><label for="nonstandard_container">{lng[Nonstandard container is used for a shipment]}</label></b></td>
  </tr>
...
<tr>
    <td colspan="2"><br /><h3>{lng[Additional charges]}</td>
</tr>

<tr>
  <td><label for="send_insured_value"><b>{lng[Send package cost to calculate insurance]}:</b></label></td>
...
<tr>
  <td><b>{lng[Handling charge amount]}:</b></td>
  <td>
...
<tr>
    <td colspan="2"><br /><h3>{lng[SmartPost Shipping settings]}</h3></td>
</tr>

<tr>
  <td><label for="add_smartpost_detail"><b>{lng[Add SmartPost Shipping data to request]}:</b></label></td>
...
<tr <?php if ($shipping_options['fdx']['add_smartpost_detail'] != "Y") echo ' style="display: none;"'; ?> class="smartpost_block">
  <td><b>{lng[Specify the indicia type]}:</b></td>
  <td>
...
<tr <?php if ($shipping_options['fdx']['add_smartpost_detail'] != "Y") echo ' style="display: none;"'; ?> class="smartpost_block">
  <td><b>{lng[Specify an endorsement type]}:</b></td>
  <td>
...
<tr<?php if ($shipping_options['fdx']['add_smartpost_detail'] != "Y") echo ' style="display: none;"';?> class="smartpost_block">
  <td><b>{lng[Specify the HubID]}:</b></td>
  <td>
...
<button type="submit" name="update_options">{$lng.lbl_apply|escape}</button>
...
?>
<a href="{$current_location}/admin/configuration/Shipping">{lng[Fedex is not enabled]}</a>
...
<tr>
  <td><label for="param11"><b>{lng[Split the shipment into multiple packages if its weight/dimensions exceed the limitations]}:</b></label></td>
...
<tr>
  <td><b>{lng[Maximum package weight]} (<?php echo $config['General']['weight_symbol']; ?>)*:</b></td>
  <td>
...
<!-- End package limits -->
<tr>
  <td><b>{lng[Girth (required for non-rectangular Priority Mail large pieces)]} (<?php echo $config['General']['dimensions_symbol']; ?>):</b></td>
  <td nowrap="nowrap">
...
<tr>
  <td><label for="status_new_method"><b>{lng[Automatically enable new shipping methods from shipping server response]}:</b></label></td>
...
<tr>
  <td><b>{lng[Shipping cost conversion rate]}:</b><br />
...
<tr>
  <td colspan="2"><h3>{lng[International U.S.P.S.]}</h3></td>
</tr>

<tr>
  <td width="50%"><b>{lng[Type of mail]}:</b></td>
  <td>
...
<tr>
  ...
  <select name="value_of_content_type" id="value_of_content_type">
    ...
    <option value="disabled"{if $shipper_options.usps.param07 == "disabled" or !$shipper_options.usps.param07) echo 'selected="selected"'; ?>>{lng[Disabled]}</option>
    <option value="fixed_value"{if $shipper_options.usps.fixed_value == "Y") echo 'selected="selected"'; ?>>{lng[Fixed value]}</option>
  </select>
...
<tr>
  <td><b>{lng[Container (International Rates)]}:</b></td>
  <td>
...
<tr>
  <td colspan="2"><h3>{lng[Domestic U.S.P.S.]}</h3></td>
</tr>

<tr>
  <td><b>{lng[Services]}:</b></td>
  <td><div style="line-height: 170%;"><a href="javascript: void(0);" onclick="$('#selected_services option').prop('selected', true);">{lng[Select all]}</a></div>
...
<tr>
  <td><b>{lng["Ground Only" indicator for "Standard Post" service]}:</b></td>
  <td>
  <select name="ground_only">
    <option value="true"<?php if ($shipping_options['usps']['ground_only'] == "true") echo 'selected="selected"'; ?>>{lng[Ground transportation required]}</option>
    <option value="false"<?php if ($shipping_options['usps']['ground_only'] == "false") echo 'selected="selected"'; ?>>{lng[Ground transportation not required]}</option>
  </select>
  </td>
</tr>

<tr>
  <td><b>{lng[Machinable]}:</b></td>
  <td>
  <select name="machinable">
    <option value="FALSE"<?php if ($shipping_options['usps']['param02'] == "FALSE") echo 'selected="selected"'; ?>>{lng[No]}</option>
    <option value="TRUE"<?php if ($shipping_options['usps']['param02'] == "TRUE") echo 'selected="selected"'; ?>>{lng[Yes]}</option>
  </select>
  </td>
</tr>

<tr>
  <td><b>{lng[Container (Express Mail)]}:</b></td>
  <td>
  <select name="container_express">
    <option value="None">{lng[None]}</option>
...
<tr>
  <td><b>{lng[Container (Priority Mail)]}:</b></td>
  <td>
...
<tr>
  <td colspan="2"><button type="submit">{lng[Apply]}</button></td>
</tr>
...
<tr>
  <td width="40%"><b>{lng[Type of delivery]}:</b></td>
  <td>
...
<tr>
  <td><b>{lng[Nature of Shipment Contents]}:</b></td>
  <td>
...
<tr>
  ...
  <td>
    <input type="text" name="weight" size="6" value="<?php echo $shipping_options['intershipper']['param09']; ?>"/> ({lng[Should not exceed]} <?php echo $max_intershipper_weight.' '.$config['General']['weight_symbol']; ?>)
  </td>
</tr>
...
<tr>
  <td><label for="customer_number"><b>{lng[Your Canada Post commercial customer number]}:</b></label></td>
...
<tr>
  <td><b>{lng[Shipping cost convertion rate]}:</b><br />
  <small>The shipping cost is always returned in US Dollars. So you need to specify the conversion rate to convert the shipping cost returned by shipping service into the necessary currency.</small>
  </td>
...
<tr>
    <td colspan="2"><br /><h3>{lng[Package options]}</h3></td>
</tr>

<tr>
  <td width="30%"><b>{lng[Options]}:</b></td>
  <td width="70%">
...
<tr>
  <td width="50%"><b>{lng[Coverage]}:</b></td>
  <td>
...
<tr>
  <td width="50%"><b>{lng[Packaging type]}:</b></td>
  <td width="50%">
  <select name="param00">
...
<tr>
  <td width="50%"><b>{lng[Ship in a specified number of days]}:</b></td>
...
<tr valign="top">
  <td width="50%"><b>{lng[Additional protection type]}:</b></td>
  <td width="50%">
  <select name="param05">
...
<tr>
  <td width="50%"><b>{lng[COD value in $]}:</b></td>
  <td><input type="text" name="param09" size="10" value="<?php echo $shipping_options['arb']['param09']; ?>" /></td>
</tr>

<tr>
  <td width="50%"><b>{lng[Maximum package weight]} (<?php echo $config['General']['weight_symbol']; ?>)*:</b></td>
  <td><input type="text" name="param10" size="10" value="<?php echo $shipping_options['arb']['param10']; ?>" />({lng[Should not exceed]} <?php echo $max_arb_weight.' '.$config['General']['weight_symbol']; ?>)</td>
</tr>
...
<tr>
  <td><label for="param11"><b>{lng[Always use the same package dimensions as specified in the "Maximum package dimensions" fields]}:</b></label></td>
...
<tr>
  <td><b>{lng[Shipping cost convertion rate]}:</b>
  </td>
...
<tr>
  <td><label for="param05"><b>{lng[Always use the same package dimensions as specified in the "Maximum package dimensions" fields]}:</b></label></td>
...
<table>
<tr>
  <td><b>{lng[Warehouse name]}:</b></td>
  <td>&nbsp;</td>
  <td><?php echo $seller_address['company_name']; ?></td>
  <td>&nbsp;&nbsp;<a href="{$current_location}/admin/configuration/Shipping#anchor_1800c_username">{lng[Change info]}</a></td>
</tr>
<tr>
  <td><b>{lng[City]}:</b></td>
  <td>&nbsp;</td>
...
<br />
<button type="button" {if $send_is_avail} onclick="forms.send_to_1800c.submit()"{else} disabled="disabled"{/if} />{$lng.lbl_1800c_send_info|escape}</button>
<?php
	} else {
?>
<br />
<a href="{$current_location}/admin/configuration/Shipping">{lng[1-800Courier options are empty]}</a>
<?php
	}
}