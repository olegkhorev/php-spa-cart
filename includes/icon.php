...
$dir = SITE_ROOT . '/var/photo/category/';
if (!is_dir($dir)) {
	mkdir($dir);
...
if (!file_exists($new_file)) {
	$file = SITE_ROOT . '/photos/category/'.$image['categoryid'].'/'.$image['iconid'].'/'.$image['file'];
...
if ($image['only_url'] == 'Y') {
	...
	if ($image['link']) {
		if (!$image['url']) {
			$image['url'] = $current_location.'/photos/category/'.$image['categoryid'].'/'.$image['iconid'].'/'.$image['file'];
		}

		echo '<a href="'.$image['url'].'"'.($image['blank'] == 'Y'?' target="_blank"':'').'>';
	}

	echo '<img'.($image['link'] ? ' huge="'.$current_location.'/photos/category/'.$image['categoryid'].'/'.$image['iconid'].'/'.$image['file'].'"' : '').' src="'.str_replace(SITE_ROOT, $current_location, $dir).'/'.$image['file'].'" alt="'.escape($image['alt']).'"'.($image['class'] ? ' class="'.$image['class'].'"':'').''.($image['id'] ? ' id="'.$image['id'].'"':'').' />';
	if ($image['link']) {
		echo '</a>';
	}
}
