...
if (!file_exists($new_file)) {
	$file = SITE_ROOT . '/photos/brand/'.$image['brandid'].'/'.$image['imageid'].'/'.$image['file'];
...
if ($image['only_url'] == 'Y') {
	...
	$image['url'] = $current_location.'/photos/brand/'.$image['brandid'].'/'.$image['imageid'].'/'.$image['file'];
	if ($image['link']) {
	...
	if ($image['center']) {
		...
		echo '<img style="margin: '.$valign.'px '.$center.'px;" src="'.str_replace(SITE_ROOT, $current_location, $dir).'/'.$image['file'].'" alt="'.escape($image['alt']).'"'.($image['class'] ? ' class="'.$image['class'].'"':'').''.($image['id'] ? ' id="'.$image['id'].'"':'').' />';
	} else
